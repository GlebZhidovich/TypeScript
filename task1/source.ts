...
const rounding: (value: number, increment: number) => number = (value, increment) => round(value / increment) * increment;
...
interface Settings extends Options, Defaults{
...
type CurVal = Currency | number | string;

interface CurrencyConstructor {
  new(value: CurVal, opts?: object): Currency;
  (this: Currency, value: CurVal, opts?: object): Currency;
}
...
function currency(
  ...
  if (!(that instanceof currency)) {
    return new (currency as CurrencyConstructor)(value, opts);
  }

  let settings: Settings = Object.assign({}, defaults, opts)
    , precision: number = pow(settings.precision)
    , v: number | string = parse(value, settings);

  that.intValue = v;
  that.value = <number>v / precision;
...
function parse(value: CurVal, opts, useRounding = true): number {
  ...
  if (isNumber || value instanceof currency) {
    v = ((isNumber ? (value as number) : ((value as Currency).value as number)) * precision);
    ...
    v = parseInt(value
        ...
        .replace(decimalString, '.'), 10)  // convert any decimal values
...
(currency as CurrencyConstructor).prototype = {
  ...
  distribute(count: number) {
    ...
    for (; count !== 0; count--) {
      let item: Currency | number = currency(split / _precision, _settings);
